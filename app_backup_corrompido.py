...
# Cache para armazenar dados carregados e timestamps
_data_cache = {}
_cache_timestamps = {}
...
def load_data(filename, use_cache=True):
    """Carrega dados do arquivo JSON com cache para melhor performance"""
    ...
    try:
        ...
        print(f"Carregado {filename} em {load_time:.3f}s")
        return data
...
def save_data(filename, data):
    """Salva dados no arquivo JSON e limpa o cache correspondente"""
    start_time = time.time()
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2, default=str)
        
        # Limpar cache deste arquivo
        clear_cache(filename)
        
        save_time = time.time() - start_time
        print(f"Salvo {filename} em {save_time:.3f}s")
...
def init_data():
    ...
    for uid, user in users.items():
        if user.get('login') == 'RafaelPinho' and user.get('tipo') == 'admin':
            admin_exists = True
            break
    ...
    if not test_exists:
        ...
        test_user = {
            'id': max([int(uid) for uid in users.keys()] + [0]) + 1,
        ...
        users[str(test_user['id'])] = test_user
        save_data(USERS_FILE, users)
        print(f"Usuário de teste criado: {test_login} / {test_senha}")
    
    # Testar hash
    test_hash = generate_password_hash(test_senha)
    print(f"Teste hash gerado: {test_hash[:50]}...")
    print(f"Verificação do hash: {check_password_hash(test_hash, test_senha)}")
    
    if admin_exists:
        print("[INFO] Administrador padrão já existe")
    
    # Inicializar outros dados
    ...
    if not load_data(GREETINGS_FILE):
        ...
        save_data(GREETINGS_FILE, default_greeting)
...
def user_dashboard():
    ...
    current_mtime = max(os.path.getmtime(USERS_FILE), os.path.getmtime(CATEGORIES_FILE), os.path.getmtime(FIELDS_FILE))
...
@app.route('/admin/download-template')
                ...
                # Pré-compilar colunas de campos adicionais para melhor performance
                additional_columns = {}
                for category, category_fields in fields.items():
                    for field_name in category_fields.keys():
                        column_with_category = f"{field_name} [{category}]"
                        if column_with_category in df.columns:
                            additional_columns[column_with_category] = (category, field_name)
                
                print(f"Índices criados em {time.time() - start_time:.3f}s")
                
                # Otimização: processar em lotes
                batch_size = 100
                processed_count = 0
                updated_count = 0
                created_count = 0
                    ...
                    for index, row in batch_df.iterrows():
                        ...
                        if existing_user:
                            ...
                            if upload_mode == 'update':
                                ...
                                if pd.notna(row['nome_completo']) and str(row['nome_completo']).strip():
                                    user_data['nome_completo'] = str(row['nome_completo']).strip()
                                    print(f"Nome atualizado: {user_data['nome_completo']}")
                                
                                if pd.notna(row['login']) and str(row['login']).strip():
                                    old_login = user_data.get('login', 'N/A')
                                    user_data['login'] = str(row['login']).strip()
                                    print(f"Login atualizado: {old_login} -> {user_data['login']}")
                                
                                if pd.notna(row['tempo_exibicao']) and str(row['tempo_exibicao']).strip():
                                    user_data['tempo_exibicao'] = str(row['tempo_exibicao']).strip()
                                    print(f"Tempo exibição atualizado: {user_data['tempo_exibicao']}")
                                
                                # Debug específico para senha
                                senha_excel = str(row['senha']).strip() if pd.notna(row['senha']) else ''
                                if senha_excel:
                                    old_hash = user_data.get('senha', 'N/A')[:30] + '...' if user_data.get('senha') else 'N/A'
                                    user_data['senha'] = generate_password_hash(senha_excel)
                                    new_hash = user_data['senha'][:30] + '...'
                                    print(f"SENHA ATUALIZADA: {senha_excel}")
                                    print(f"Hash anterior: {old_hash}")
                                    print(f"Hash novo: {new_hash}")
                                else:
                                    print(f"SENHA NÃO ATUALIZADA (campo vazio ou NaN)")
                                    print(f"Valor da senha no Excel: '{row['senha']}' (NaN: {pd.isna(row['senha'])})")
                            ...
                            else:  # upload_mode == 'replace'
                                # Modo REPLACE: substitui todos os campos
                                print(f"Processando usuário existente (REPLACE): {user_data.get('login', 'N/A')} (Matrícula: {matricula})")
                                
                                user_data['nome_completo'] = str(row['nome_completo']).strip() if pd.notna(row['nome_completo']) else ''
                                user_data['login'] = str(row['login']).strip() if pd.notna(row['login']) else ''
                                user_data['tempo_exibicao'] = str(row['tempo_exibicao']).strip() if pd.notna(row['tempo_exibicao']) else ''
                            
                                # Debug específico para senha no modo REPLACE
                                senha_excel = str(row['senha']).strip() if pd.notna(row['senha']) else ''
                                if senha_excel:
                                    old_hash = user_data.get('senha', 'N/A')[:30] + '...' if user_data.get('senha') else 'N/A'
                                    user_data['senha'] = generate_password_hash(senha_excel)
                                    ...
                                    print(f"SENHA NÃO ATUALIZADA (REPLACE - campo vazio ou NaN)")
                                    print(f"Valor da senha no Excel: '{row['senha']}' (NaN: {pd.isna(row['senha'])})")
                                    # No modo replace, se não tiver senha, manter a antiga
                                    if not user_data.get('senha'):
                                        user_data['senha'] = generate_password_hash('temp123')
                                        print(f"Senha temporária criada: temp123")
                                        ...
                                        if column_with_category in df.columns:
                                            ...
                                            if field_config.get('type') in ['date', 'data'] and '/' in str(value):
                                                try:
                                                    ...
                                                    pass
                                            user_data['campos_adicionais'][category][field_name] = str(value)
                                    # Se estiver vazio, mantém vazio (limpa o dado existente)
                                    elif field_name in df.columns:
                                        # Compatibilidade com formato antigo
                                        value = row[field_name]
                                        if pd.notna(value) and str(value).strip():
                ...
                # Verificar se os usuários foram realmente atualizados
                print("VERIFICAÇÃO PÓS-UPLOAD:")
                for uid, user in users.items():
                    if user.get('matricula') in ['60680', '60667', '60690']:
                        print(f"  {user.get('matricula')}: {user.get('login')} - OK")
...
def admin_download_template():
    ...
    return send_file(template_path, as_attachment=True, download_name='template_usuarios.xlsx')
